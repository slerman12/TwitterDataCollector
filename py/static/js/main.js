        ...
        $('.tile').each(function (i) {
            $( this ).click(function () {
                if(data.jobStatus.jobs[i].status === 'quoted'){
                    ...
                    $('#myModal .modal-body').html('<p><strong>Enter the following into your computer\'s command line to download the Twitter data.</strong> Make sure you are in the directory in which you want the files to be downloaded.</p><pre><code>curl -sS -u' + username + ':'+password+' https://historical.gnip.com/accounts/'+ account +'/publishers/twitter/historical/track/jobs/' + data.jobStatus.jobs[i].uuid + '/results.csv | xargs -P 8 -t -n2 curl -o</code></pre><em>You must download the files within 15 days. Afterwards, GNIP deletes them from their servers.</em>');
                }
                $('#myModal').modal('show');
